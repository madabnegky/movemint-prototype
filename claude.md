...
### Fonts
GT America is self-hosted under `public/fonts/` and registered via `@font-face` in `src/app/globals.css` (deliberately not `next/font/local` — see the comment there): **Standard** → family `"GT America"` (body, `font-sans`), **Extended** → family `"GT America Extended"` (titles, `font-extended`). Four `.woff2` files are needed — Standard/Extended × Regular/Medium. See `public/fonts/README.md` for the exact filenames.
...
- **A `git push` / auto-deploy renders system-sans fallback, not GT America.** The build succeeds (a missing `/fonts/*.woff2` just 404s and `@font-face` degrades gracefully), but the licensed font is absent.
